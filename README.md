...
_____
ls
ls -la - показывает список всех файлов, в т.ч. скрытых системных
git clone	Копируем адрес удалённого репо
Но для этого сначала надо создать новую папку для него в лок репо GIT			
Потом в баш перейти в ту папку cd . .			
git push origin main	Добавить файлы в удаленный репо на гитхаб		

cd C://путь	Перейти в папку с репом (GIT)				
					
Создаем обычным способом папку или файл в локальном репо (например test)					
C://путь/test	Переходим в эту папку через баш (находясь в папке GIT)				
					
git init	Создать репо. Создасться папка .git В этой папке самый важный файл config				
Сделать видимыми скрытые системные файлы					
					
Создаём пользователя (добавляем его в local)					
git config user.name "имя пол-ля"					
git config user.email почта					
git config --list	Посмотреть что есть в файле config				
git config --list --local - посмотреть настройки конфига локал					
git config --list --global - посмотреть настройки конфига глобал					
git init

git status	Показывает текущую ситуацию в репозитории и показ ветку, в кот находимся
git add - индексация файлов в репо
git add .	Индексирует все непроиндекс. файлы
git add название файла	Опр файл

git config --global user.name "имя"	Добавляем пол-ля в global				
git config --system	Добавляем пол-ля в system 				
git config --global user.email почта	Добавляем почту пол-ля в global				
git config --unset user.name	Удалить дублирование юзера в локал				
git config --unset user.email					
git config --unset global user.name	Удалить юзера  из глобал				
git status	Показывает текущую ситуацию в репозитории и показ ветку, в кот находимся				
					
Добавляем вручную файлы в папку с репо					
git add - индексация файлов в репо					
git add .	Индексирует все непроиндекс. файлы				
git add название файла	Опр файл				
git restore --staged название файла	Отменить индексацию				
git restore --staged .	Отменить индексацию всех файлов				
Далее нужно отправить файлы в удаленный репо - комиттим					
git commit -m "комментарий, например add files"					


git help -a	Список команд	Если добавить после команды -h то покажет помощь по конкретной команде				
выход - Q						
git -v	Версия гита					
Посмотреть последние команды - нажать стрелочку вверх

выйти из лога - q						
git log -p	Покажет больше изменений, например строки которое были добавлены (+) или удалены (-)					
git log --oneline	Удобнее отображение					
git diff	Показывает изменения, в проиндексированных файлах					
git diff staged	Показывает изменения в не проиндексированных файлах

"ls - покажет список файлов в директории, в которой мы находимся (всё, кроме скрытых)
ls -la - покажет список файлов в директории, в которой мы находимся (в том числе скрытые)"

"git commit -h Покажет опции определенной команды. В данном случае commit				"

"Выйти из vim - Esc : w q Enter		"
...
>Добавление файла без индексации, то есть пропуская этот этап, он автоматич проиндексируется ИСПРАВИТЬ Индексация и комит уже подготовленного к комиту, но изменённого файла file3.txt. Он добавит только те файлы, которые уже были проиндексированы когда-то. Новые - нет.
>Если файл новый то сначала add, затем git commit m "коммент"
>
>![image](https://user-images.githubusercontent.com/97261554/236724873-05122144-42fc-4c89-9199-73c04748923f.png)

```git commit --amend```
>Докидывает инфу в предыдущий комит (сделали изменения в файлах/добавили файлы, 
проиндексили их и этой командой докинули их в пред комит), так же можно исправить комментарий пред комита"
